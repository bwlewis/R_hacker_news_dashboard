...
# Startup: collect N most recent stories and total post rate
N <- 50
...
observe({
  isolate({
    ...
    dt <-  60 * as.numeric(difftime(Sys.time(), state$time, units="secs"))
    state$time <- Sys.time()
    state$latest <- latest()
  })
...
renderUI({
  col <- "black"
  HTML(sprintf("<div style='text-align: center; color: white; background-color: %s; font-size: 32px;'>%s</div>", col, state$time))
})
```
